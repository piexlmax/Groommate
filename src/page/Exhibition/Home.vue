<template>
  <div>
      <mu-appbar title="图册" class="Top-Class" />
      ...
      <div style="position: relative" v-for="(slide,index) in swiper_data" :key="index">
        <a>
          ...
          <span class="title-mask" v-if="slide.title">
            <p>by {{slide.name}}</p>
            <p>{{slide.title}}</p>
            <mu-flat-button to="/exhibition/atlas" class="more-btn" labelPosition="before"  label="更多" icon="more_vert" primary style="color:#ff4081"/>        
          </span>
        </a>
      </div>   
    </slider>
    ...
    <div class="topic-class" @click="To_Potic">
        <div class="topic-class-background">
            ...
            <span class="topic-class-content">
                ...
                <p style="font-size: 1.2em">#北京好室友#</p>
                <p style="color: #C9C9C9">北京好室友北京好室友北京好室友北京好室友北京好室友北京好室友</p>
            </span>
